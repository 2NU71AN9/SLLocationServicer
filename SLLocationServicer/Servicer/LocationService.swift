...
final class LocationService: NSObject {
    ...
    @objc var afterUpdatedLocationAction: ((LocationModel?) -> Void)?
    ...
    private lazy var locationManager: CLLocationManager = {
        ...
        // 重新定位的最小变化距离。不影响电量，位移信息不是一条直线而是有很多锯齿，高精度的 distanceFilter 可以减少锯齿，给你一个更精确地轨迹，但是，太高的精度值会让你的轨迹像素化(看到很多马赛克)，所以10m是一个相对比较合适的值
...
extension LocationService: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        geocoder.reverseGeocodeLocation(newLocation, completionHandler: { [weak self] (placemarks, error) in
            DispatchQueue.global().sync { [weak self] in
                if (error != nil) {
                    print("self reverse geocode fail: \(error!.localizedDescription)")
